...
namespace TesApi.Web
{
    public class LoggingExecutor
    {
        ...
        public async Task Execute(string actionDescription, string referenceId, Func<Task> action)
        {
            ...
            {
                logger.LogTrace($"TIMING_EXCEPTION,{start:s},{actionDescription},{referenceId},{DateTime.UtcNow.Subtract(start).TotalSeconds},{ex.Message}");
                throw;
            }

            logger.LogTrace($"TIMING,{start:s},{actionDescription},{referenceId},{DateTime.UtcNow.Subtract(start).TotalSeconds}");
        }
